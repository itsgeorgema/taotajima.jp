...
void main()	{
	...
	// Double the fold width by dividing spatial components by 2 (keeping the same angle and smooth transitions)
	float delayValue = p*7. - (uv.y*2.)/2. + uv.x/2. - 2.;
	...
	// Add subtle shadow overlay to darken the videos slightly
	float shadowIntensity = 0.25; // Adjust this value (0.0 = no shadow, 1.0 = black)
	rgba.rgb *= (1.0 - shadowIntensity);
